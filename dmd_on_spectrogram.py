...
class LowFreqBandpassImpulsiveVSHighFreqBandpassImpulsive:
    def __init__(
        ...
        faulty_band=[
            0.4,
            0.7,
        ],  # Band as fraction of nyquist (7000Hz is around 7000/(20480/2) = 0.68)
    ...
    def get_reference_component(self):
        return self.get_impulses(
            self.peak_ref, self.f_ref, epsilon=1e-1
        ) * self.get_bandpass_noise(self.reference_band[0], self.reference_band[1])

    def get_faulty_component(self):
        return self.get_impulses(
            self.peak_fault, self.f_fault, epsilon=1e-2
        ) * self.get_bandpass_noise(self.faulty_band[0], self.faulty_band[1])


f_fault = 108.1  # Hz (Bearing fault frequency, BPFO)

fs = 40960
sig_data = LowFreqBandpassImpulsiveVSHighFreqBandpassImpulsive(
    f_fault=f_fault, sig_duration=0.5, fs=fs
)
# Get fautly signal
...
f, t, Sxx = spectrogram(
    faulty_signal, fs=fs, nperseg=window_length, noverlap=int(0.9 * window_length)
)
