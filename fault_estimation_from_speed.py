...
relative_displacement = scipy.integrate.cumtrapz(
    signal, dx=1 / fs, initial=0
)  # In relative revolutions : revs/sec becomes revs

# Get a mapping between angle (revs) and the response signal
signal_as_function_of_angle = scipy.interpolate.interp1d(
    relative_displacement, signal, kind="linear"
)

# Define a constant relative displacement angle
constant_relative_displacement_angle = np.linspace(
    relative_displacement[0], relative_displacement[-1], len(signal)
)  # Also relative revolutions

angle_sample_rate = 1 / (np.mean(np.diff(relative_displacement)))

resampled_signal = signal_as_function_of_angle(
    constant_relative_displacement_angle
)  # Resample the signal to have constant angle

# Detrend signal using savgol filter
resampled_signal = resampled_signal - scipy.signal.savgol_filter(
    resampled_signal, 501, 3
)
...
fig = go.Figure(
    ...
    layout=go.Layout(
        ...
        xaxis=dict(title="Number of revolutions"),
        yaxis=dict(title="De-trended, order tracked angular velocity (rps)"),
    ),
)
# fig.show()
...
fig = go.Figure(
    ...
    layout=go.Layout(
        ...
        xaxis=dict(
            title="Angular frequency (events/relative revolution)", range=[0, 10]
        ),
