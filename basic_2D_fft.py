...
class WaveGenerator(object):
    def __init__(self, wave_number_as_function_of_t, frequency_as_function_of_t,component_phases):
    ...
    def generate_wave_data(self,n_timesteps, n_locations):
        ...
        for t_index in range(n_timesteps): # Loop through all time steps
            for wave_number, frequency,phase in zip(self.wave_number_as_function_of_t(time[t_index]),
                                                    self.frequency_as_function_of_t(time[t_index]),
                                                    self.component_phases
                                                    ): # Add each frequency component together
                wave_data[t_index,:] += np.sin(wave_number*space*2*np.pi/n_locations + frequency*time[t_index]*2*np.pi/n_timesteps + phase)
            ...
            layout=go.Layout(
                ...
                updatemenus=[dict(
                    ...
                    buttons=[dict(label="Play",
                                    method="animate",
                                    args=[ None, {"frame": {"duration": 100,# 100*wave_data.shape[0],
                                                            "redraw": True},
...
class LocalLinearSpeedEstimator(object):
    ...
    def get_fft_2D_magnitude(self,window_time=True, window_space=True):
        ...
        if window_time:
            wave_data = wave_data * np.tile(np.hanning(self.n_time_steps)[:,None], (1, self.n_locations))
        if window_space:
            wave_data = wave_data * np.tile(np.hanning(self.n_locations), (self.n_time_steps, 1))
        ...
        wave_numbers = np.fft.fftfreq(self.wave_data.shape[0], d=1/self.wave_data.shape[0])
        frequencies = np.fft.fftfreq(self.wave_data.shape[1], d=1/self.wave_data.shape[1])
...
if __name__ == "__main__":
    ...
    constant_wave_numbers =   (0.01 +   0.01*np.random.rand(
        n_components) ) *n_space # np.array([1/n_space]) #0.1+0.2 * np.random.rand(n_components) #  np.array([1/100]) # Oscillations/spatial sample
    ...
    phase_velocity_as_function_of_t = lambda t: 1#1 # Spatial samples traversed per temporal sample
    print("wave numbers: ", constant_wave_numbers)
    print("true phase velocity at t=0: ", phase_velocity_as_function_of_t(0))

    wave_number_as_function_of_t = lambda t: constant_wave_numbers # Spatial oscillations per spatial sample

    # v = omega/k, so omega = v*k
    frequency_as_function_of_t = lambda t: constant_wave_numbers * phase_velocity_as_function_of_t(t) # Temporal oscillations per temporal sample
    wave_generator = WaveGenerator(wave_number_as_function_of_t, frequency_as_function_of_t, random_phases)
