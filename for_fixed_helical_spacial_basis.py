...
G = np.column_stack([G, np.real(G_hel_wave), np.imag(G_hel_wave)])  # Take real/imag for real basis (or keep complex)
...
noise_hel = noise_level * np.linalg.norm(X_hel_clean) / np.sqrt(n * m) * (np.random.randn(n, m) + 1j * np.random.randn(n, m))
...
print(f"Helical reconstruction error (relative Frobenius norm): {hel_error:.4f} ({hel_error*100:.2f}%)")

# Visualization: Helical spatial modes and space-time
fig_hel, axes_hel = plt.subplots(2, 3, figsize=(15, 10))
fig_hel.suptitle(f'Prescribed Spatial Basis: Helical Gear (α={alpha:.0f} rad, κ={kappa:.2f} rad/pix)')
...
axes_hel[1, 0].plot(s, np.real(X_hel_recon[:, k_sample]), 's-', label='Reconstructed (Prescribed Spatial)', linewidth=2)
...
im_hel_st = axes_hel[1, 1].pcolormesh(s, theta_period, np.real(X_hel_fund_period), cmap='RdBu_r', shading='gouraud', vmin=-1.5, vmax=1.5)
...
im_theta = axes_hel[1, 2].imshow(np.abs(Theta), aspect='auto', cmap='viridis', extent=[0, r, 0, q])
