...
omega_1 = 2 * np.pi / P
omegas = np.array(
    [j * omega_1 for j in range(1, r + 1)]
...
phi_2 = np.tanh((s - 20) / 5) + 1j * np.exp(
    -((s - 25) ** 2) / 50
)  # Real: step; Imag: Gaussian bump
...
b = np.array(
    [1.0, 0.8 * 1j, 0.6]
)  # Absorb into tilde{phi} later, but use here for generation
...
print(
    f"Reconstruction error (relative Frobenius norm): {recon_error:.4f} ({recon_error * 100:.2f}%)"
)
...
for j in range(r):
    ...
    ax.set_title(
        f"Estimated Spatial Mode {j + 1} (ω_{j + 1} = {omegas[j]:.2f} rad/frame)"
    )
...
ax_recon[0].plot(
    s_plot, X_recon[:, k_sample].real, "s-", label="Reconstructed (Real)", linewidth=2
)
...
ax_recon[1].plot(
    s_plot, X_recon[:, k_sample].imag, "s-", label="Reconstructed (Imag)", linewidth=2
)
...
s_fixed = 25
ax_recon[2].plot(
    np.arange(m), np.abs(X[s_fixed, :]), "o-", label="True |x(s_fixed, t)|", alpha=0.7
)
ax_recon[2].plot(
    np.arange(m),
    np.abs(X_recon[s_fixed, :]),
    "-",
    label="Reconstructed |x(s_fixed, t)|",
    linewidth=2,
)
...
for j in range(r):
    axes_true[j].plot(
        s_plot,
        (Phi_true * b[None, j])[:, j].real,
        label="True Real(φ_j * b_j)",
        linewidth=2,
    )
    axes_true[j].plot(
        s_plot, Phi_est[:, j].real, "--", label="Est. Real(φ̃_j)", linewidth=2
    )
    axes_true[j].plot(
        s_plot, (Phi_true * b[None, j])[:, j].imag, label="True Imag(φ_j * b_j)"
    )
...
fig_st.suptitle(
    f"Space-Time Diagram: 2π Evolution of Fundamental Mode (ω_1 = {omega_1:.3f} rad/frame)"
)

# Left: True noisy data over one period (real part)
im1 = ax1.pcolormesh(
    S, Theta, np.real(X_period), cmap="RdBu_r", shading="gouraud", vmin=-1.5, vmax=1.5
)
