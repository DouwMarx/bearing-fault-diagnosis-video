...
for harmonic in range(2, 5):  # Add 2nd, 3rd, and 4th harmonics
    f1 += (1 / harmonic) * np.sin(harmonic * omega_1 * t[:, np.newaxis] - harmonic * k_1 * x[np.newaxis, :])
...
phase = np.mod(envelope_freq * t[:, np.newaxis] / (2 * np.pi), 1.0)  # Normalize to [0,1]
temporal_envelope = np.where(phase < duty_cycle, 1.0, 0.0)

# Create spatial mask (only active in 10% of spatial domain)
spatial_mask = np.where(np.abs(x[np.newaxis, :] - spatial_center) <= spatial_extent * L / 2, 1.0, 0.0)
...
for i, (m, n) in enumerate(zip(dominant_modes[0], dominant_modes[1])):
    ...
    if abs(k_fourier) > 1e-10:
        ...
        print(f"Mode {i}: m={m}, n={n}, ω={omega_fourier:.3f}, k={k_fourier:.3f}, v={v_fourier:.3f}, |F|={F_mag[m,n]:.3f}")
    else:
        print(f"Mode {i}: m={m}, n={n}, ω={omega_fourier:.3f}, k={k_fourier:.3f}, k≈0 (DC), |F|={F_mag[m,n]:.3f}")
...
for i, (m, n, omega_f, k_f, v_phase, mag) in enumerate(filtered_modes):
    print(f"Filtered {i}: m={m}, n={n}, ω={omega_f:.3f}, k={k_f:.3f}, v={v_phase:.3f}, |F|={mag:.3f}")
