...
frames_polar_array = np.load(
    "rpm_variable_23723_frames_0.15_channels_1_space_polar.npy"
)
print("Dimensions of the polar transformed frames: ", frames_polar_array.shape)

# Extract random sample of signals
n_samples = 5000  # 00
frames_polar_array = frames_polar_array[
    np.random.choice(frames_polar_array.shape[0], n_samples, replace=False), :, :
]
...
frames_polar_array_norm = (
    frames_polar_array - np.mean(frames_polar_array, axis=0)
) / np.std(frames_polar_array, axis=0)
...
convolved = convolve1d(
    frames_polar_array_norm, filter, axis=2, mode="constant", cval=0.0
)
...
fig.add_trace(
    go.Scatter(
        x=np.arange(sdev.shape[0]),
        y=sdev / sdev.max() * frames_polar_array_norm[0].shape[0],
        mode="lines",
        name="Filter variance for random angles and time steps",
    )
)
...
for i, peak in enumerate(peak_indices[0]):
    fig.add_trace(
        go.Scatter(
            x=[peak, peak],
            y=[0, frames_polar_array_norm[0].shape[0]],
            mode="lines",
...
fig.update_layout(
    title="Identify independent regions of interest",
    xaxis_title="Radial axis (Channel)",
    yaxis_title="Angle (0-360 degrees)",
)
fig.show()
