...
f = (0.4 * np.sin(omega1 * t[:, np.newaxis] - k1 * x[np.newaxis, :]) +
     0.8 * np.sin(omega2 * t[:, np.newaxis] - k2 * x[np.newaxis, :]) +  # Make target wave stronger
...
plt.subplot(131)
plt.imshow(np.abs(F_shift), extent=[k_x_shift.min(), k_x_shift.max(), freq_t_shift.min(), freq_t_shift.max()],
...
plt.subplot(132)
plt.imshow(f, extent=[x.min(), x.max(), t.min(), t.max()], aspect='auto', origin='lower')
...
plt.subplot(133)
plt.imshow(f_recon, extent=[x.min(), x.max(), t.min(), t.max()], aspect='auto', origin='lower')
plt.xlabel('Space x (m)')
plt.ylabel('Time t (s)')
        ...
        if abs(k) > 1e-10:  # Avoid division by zero
            ...
            if abs(abs(v_phase) - abs(v_target)) < tolerance:
                ...
                if filtered_count <= 10:  # Limit printed output
                    print(f"Filtered mode: m={m}, n={n}, ω={omega:.3f}, k={k:.3f}, v={v_phase:.3f}, |F|={abs(F_shifted_for_filter[m,n]):.3f}")
...
plt.subplot(132)  
plt.imshow(np.abs(F_filtered_shift), extent=[k_x_shift.min(), k_x_shift.max(), freq_t_shift.min(), freq_t_shift.max()],
           aspect='auto', origin='lower')
plt.xlabel('Wavenumber k_x (rad/m)')
plt.ylabel('Freq ω (rad/s)')
...
plt.imshow(np.abs(F_diff_shift), extent=[k_x_shift.min(), k_x_shift.max(), freq_t_shift.min(), freq_t_shift.max()],
           aspect='auto', origin='lower')
plt.xlabel('Wavenumber k_x (rad/m)')
plt.ylabel('Freq ω (rad/s)')
...
plt.subplot(122)
plt.imshow(f_filtered, extent=[x.min(), x.max(), t.min(), t.max()], aspect='auto', origin='lower')
plt.xlabel('Space x (m)')
plt.ylabel('Time t (s)')
